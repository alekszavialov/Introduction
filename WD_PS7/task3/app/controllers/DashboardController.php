...
class DashboardController
{
    ...
    public function logInAction()
    {
        ...
        if (!$user) {
            PhpResponse::ajaxResponse(401, 'Incorrect password!', __FUNCTION__);
        }
        $_SESSION['user'] = $user;
        PhpResponse::ajaxResponse(200, 'chat', __FUNCTION__);
    }
    ...
    public function logOutAction()
    {
        $this->model->setTable('user');
        $this->model->logOut();
        PhpResponse::ajaxResponse(200, 'main', __FUNCTION__);
    }

    public function addMessageAction()
    {
        if (!$this->isUser()) {
            PhpResponse::ajaxResponse(401, 'You need to login!', __FUNCTION__);
        }
        if (!$this->isCorrectMessage()) {
            PhpResponse::ajaxResponse(400, 'Incorrect message!', __FUNCTION__);
        }
        $this->model->setTable('message');
        $data = $this->model->addMessage();
        if ($data) {
            PhpResponse::ajaxResponse(200, $data, __FUNCTION__);
        }
    ...
    public function loadMessageAction()
    {
        ...
        if (!$data) {
            PhpResponse::ajaxResponse(202, '', __FUNCTION__);
        }
        PhpResponse::ajaxResponse(200, $data, __FUNCTION__);
    }
